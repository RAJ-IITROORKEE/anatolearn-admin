...
- `SUPABASE_SECRET_KEY`
- `SUPABASE_AUTH_REDIRECT_URL`, `SUPABASE_PASSWORD_RESET_REDIRECT_URL` (recovery must
  route through `/auth/callback?next=%2Freset-password` for the server-side PKCE exchange)
