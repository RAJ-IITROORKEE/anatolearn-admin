...
export async function getAdminAttemptLabels({ organSystemIds, topicIds }: { organSystemIds: readonly string[]; topicIds: readonly string[] }) {
  ...
  const [systems, topics] = await Promise.all([
    uniqueSystemIds.length ? prisma.organSystem.findMany({ where: { id: { in: uniqueSystemIds } }, select: { id: true, name: true } }) : [],
    uniqueTopicIds.length ? prisma.topic.findMany({ where: { id: { in: uniqueTopicIds } }, select: { id: true, title: true } }) : [],
  ]);
  return {
