...
export default async function OrganSystemsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <><PageHeader action={<Link className={buttonVariants()} href="/organ-systems/new"><Plus className="size-4" />Add organ system</Link>} description="Manage the top-level anatomy curriculum and publication state." eyebrow="Learning content" title="Organ systems" /><FilterBar defaultValue={input.q}><select aria-label="Status" className={fieldClass} defaultValue={input.status ?? ""} name="status"><option value="">All statuses</option><option>DRAFT</option><option>PUBLISHED</option><option>ARCHIVED</option></select></FilterBar>{result.items.length ? <><ResourceCards>{result.items.map((item) => <ResourceCard href={`/organ-systems/${item.slug}`} key={item.id} title={item.name} actions={<><StatusBadge status={item.status} /><StatusBadge status={item.isActive ? "ACTIVE" : "INACTIVE"} /></>}><div className="mb-4 grid grid-cols-2 gap-3"><AdminMediaThumbnail attached={Boolean(item.coverMediaId)} label="Cover" legacyUrl={item.coverImageUrl} media={item.coverMediaId ? media.get(item.coverMediaId) : undefined} /><AdminMediaThumbnail attached={Boolean(item.iconMediaId)} label="Icon" legacyUrl={item.iconImageUrl} media={item.iconMediaId ? media.get(item.iconMediaId) : undefined} /></div><p>{item.shortDescription}</p><p className="mt-2">Slug {item.slug} · Order {item.displayOrder} · Updated {item.updatedAt.toLocaleDateString()}</p></ResourceCard>)}</ResourceCards><div className="mt-5"><Pagination page={result.pagination.page} pageCount={Math.max(1, result.pagination.totalPages)} pathname="/organ-systems" /></div></> : <EmptyState actionHref="/organ-systems/new" actionLabel="Add organ system" description="Create the first organ system or change the current filters." title="No organ systems found" />}</>;
}
