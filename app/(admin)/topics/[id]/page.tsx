...
export default async function TopicPage({ params }: { params: Promise<{ id: string }> }) { const { id } = await params; let item; try { item = await getAdmin("topic", id); } catch { notFound(); } const systems = await listAdmin("organSystem", { page: 1, pageSize: 100, sortBy: "name", sortOrder: "asc" }); return <><PageHeader action={<div className="flex flex-wrap gap-2"><StatusBadge status={item.status} />{item.status === "DRAFT" && <InlineAction action={changeResourceStatus.bind(null, "topic", id, "PUBLISHED")}>Publish</InlineAction>}{item.status !== "ARCHIVED" && <InlineAction action={trashResourceAction.bind(null, "topic", id)} confirmMessage="This hides the topic and its content from normal views. It can be restored from Settings > Trash for 30 days. Continue?">Delete</InlineAction>}</div>} description="Edit the topic and its place in the anatomy curriculum." eyebrow="Topics" title={item.title} /><TopicForm action={updateResource.bind(null, "topic", id)} item={item} systems={systems.items.map((system) => ({ id: system.id, label: system.name }))} /></>; }
