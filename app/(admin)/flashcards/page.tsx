...
export default async function FlashcardsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <>
    ...
    <FilterBar defaultValue={input.q} placeholder="Search front, back, or notes">
    ...
    {result.items.length ? <BulkActionForm action={bulkFlashcardStatusAction}>
      <div className={cn("grid gap-4", grid && "md:grid-cols-2 xl:grid-cols-3")}>
        {result.items.map((item) => <article className="rounded-2xl border border-success/20 bg-surface p-4 shadow-sm transition hover:border-success/40 sm:p-5" key={item.id}><div className="flex items-start gap-3"><input aria-label={`Select ${item.frontText}`} className="mt-1 size-4" name="ids" type="checkbox" value={item.id} /><div className="min-w-0 flex-1"><div className="flex flex-wrap gap-2"><StatusBadge status={item.status} /><span className="rounded-full bg-success-soft px-2.5 py-1 text-xs font-bold text-success">{item.difficulty.toLowerCase()}</span></div><div className="mt-3 grid grid-cols-2 gap-3"><AdminMediaThumbnail attached={Boolean(item.frontMediaId)} label="Front" legacyUrl={item.frontImageUrl} media={item.frontMediaId ? media.get(item.frontMediaId) : undefined} /><AdminMediaThumbnail attached={Boolean(item.backMediaId)} label="Back" legacyUrl={item.backImageUrl} media={item.backMediaId ? media.get(item.backMediaId) : undefined} /></div><Link className="mt-3 block font-bold leading-6 text-foreground hover:text-success" href={`/flashcards/${item.id}`}>{item.frontText}</Link><details className="mt-3 text-sm"><summary className="cursor-pointer font-semibold text-body">Show back</summary><p className="mt-2 whitespace-pre-wrap rounded-xl bg-subtle p-3 text-muted">{item.backText}</p></details></div></div></article>)}
      </div>
      <Pagination page={result.pagination.page} pageCount={Math.max(1, result.pagination.totalPages)} pathname="/flashcards" />
    </BulkActionForm> : <EmptyState actionHref="/flashcards/new" actionLabel="Add flashcard" description="Create a flashcard or change the current filters." title="No flashcards found" />}
  </>;
}
