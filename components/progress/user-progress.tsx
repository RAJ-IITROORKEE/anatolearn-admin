import { Activity, CalendarDays, ClipboardCheck, Target } from "lucide-react";
...
export function UserProgress({ data }: { data: UserProgressData }) {
  const hasRankings = data.strengths.length > 0 || data.weaknesses.length > 0;
  return (
    <>
      <PageHeader description="Read-only learning activity and weighted assessment performance." eyebrow="User progress" title={data.profile.fullName} />
      ...
      <section className="mt-8" aria-labelledby="recent-attempts">
        <h2 className="text-xl font-bold text-foreground" id="recent-attempts">Recent attempts</h2>
