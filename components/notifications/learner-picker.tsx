...
export function LearnerPicker({ initial, initialSelected, onDirty, searchAction }: {
  ...
  return <div className="grid gap-3">
    ...
    {selectedItems.length > 0 && <div aria-label="Selected learners" className="flex min-w-0 max-h-40 flex-wrap gap-2 overflow-y-auto rounded-xl border border-border bg-surface p-2">
      {selectedItems.map((learner) => <span className="inline-flex min-h-10 max-w-full min-w-0 items-center gap-1 rounded-full bg-primary-soft pl-3 text-xs font-semibold text-primary" key={learner.id}><span className="min-w-0 break-words">{learner.fullName}</span><button aria-label={`Remove ${learner.fullName}`} className="inline-flex size-10 shrink-0 items-center justify-center rounded-full hover:bg-primary/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={() => toggle(learner, false)} type="button"><X aria-hidden="true" className="size-3" /></button></span>)}
    </div>}
    ...
    <div className="max-h-64 overflow-y-auto rounded-xl border border-border bg-surface p-2" role="group" aria-label="Active learner search results">
      {result.items.length ? result.items.map((learner) => {
        ...
        return <label className="flex min-h-11 min-w-0 items-center gap-3 rounded-lg px-2 text-sm hover:bg-subtle" key={learner.id}><span className="inline-flex size-10 shrink-0 items-center justify-center"><input aria-label={`${learner.fullName} (${learner.email})`} checked={checked} disabled={!checked && selected.size >= 500} onChange={(event) => toggle(learner, event.target.checked)} type="checkbox" /></span><span className="min-w-0"><strong className="block break-words text-foreground">{learner.fullName}</strong><span className="break-all text-muted">{learner.email}</span></span></label>;
