...
export function MiniPagination({ campaignId, current, kind, pages }: { campaignId: string; current: number; kind: "recipientPage" | "deliveryPage"; pages: number }) {
  if (pages <= 1) return null;
  const link = (page: number) => `/notifications/${campaignId}?${kind}=${page}`;
  const controlClass = buttonVariants({ variant: "outline", size: "sm" });
  return <nav aria-label={`${kind === "recipientPage" ? "Recipient" : "Delivery"} pagination`} className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-border pt-4 text-sm"><span className="text-muted">Page {current} of {pages}</span><div className="flex gap-2">{current <= 1 ? <span aria-disabled="true" className={`${controlClass} opacity-50`}>Previous</span> : <Link className={controlClass} href={link(current - 1)}>Previous</Link>}{current >= pages ? <span aria-disabled="true" className={`${controlClass} opacity-50`}>Next</span> : <Link className={controlClass} href={link(current + 1)}>Next</Link>}</div></nav>;
}
...
export function CampaignDetail({ campaign, deliveries, evidence, learners, providerReady, recipients, searchAction, selectedLearners }: {
  ...
  return <>
    ...
    {campaign.status === "DRAFT" ? <CampaignEditor action={updateCampaignAction.bind(null, campaign.id)} campaign={campaign} learners={learners} providerReady={providerReady} searchAction={searchAction} selectedLearners={selectedLearners} /> : <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(280px,.45fr)]">
      <section className={`${panelClass} min-w-0`}><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-bold text-primary">{humanize(campaign.type)}</p>{terminal && <span className="text-xs font-semibold text-muted">Immutable terminal record</span>}</div><h2 className="mt-4 break-words text-xl font-bold text-foreground">{campaign.title}</h2><p className="mt-3 whitespace-pre-wrap break-words leading-7 text-body">{campaign.message}</p></section>
