    ...
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content align="end" className="z-50 min-w-56 rounded-xl border border-border bg-surface p-1.5 shadow-lg" sideOffset={8}>
          ...
          <DropdownMenu.Item
            className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-sm outline-none focus:bg-subtle data-[disabled]:cursor-wait data-[disabled]:opacity-60"
            disabled={isLoggingOut}
            onSelect={() => startLogout(async () => logoutAction())}
          >
