...
type Option = { id?: string; optionText: string; mediaId?: string | null; isCorrect: boolean };
type Question = {
  topicId: string;
  questionText: string;
  mediaId: string | null;
...
const emptyOptions = (): Option[] => Array.from({ length: 4 }, (_, index) => ({ optionText: "", mediaId: null, isCorrect: index === 0 }));
...
export function QuestionForm({ action, assessmentType, item, topics }: { action: FormAction; assessmentType: AssessmentType; item?: Question; topics: Array<{ id: string; label: string }> }) {
  ...
  const [options, setOptions] = useState<Option[]>(item?.options ?? emptyOptions());
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,.7fr)]">
      <div className={cn(panelClass, tone === "quiz" ? "border-quiz/20" : "border-test/20")}>
        <ActionForm action={action} label={item ? "Save question" : "Create draft question"}>
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <label className={labelClass}>Question media ID <span className="font-normal text-muted">Optional</span><input className={fieldClass} defaultValue={item?.mediaId ?? ""} name="mediaId" /></label>
          </div>
